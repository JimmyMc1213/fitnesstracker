...
import { isFutureYouMediaCleared } from "./futureYouDraft";
import type { HomeFutureYouEntryMode } from "./homeFutureYouModel";
...
function hasFutureYouMedia(draft: FutureYouDraft): boolean {
  if (!isFutureYouMediaCleared(draft)) return true;
  const status = draft.generationStatus;
  return status != null && ACTIVE_GENERATION.has(status);
}

/** Subscribed user finished onboarding but has no saved NewYou preview yet. */
