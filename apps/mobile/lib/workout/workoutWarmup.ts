...
import { findExerciseInLibrary } from "./exerciseCatalogSearch";
...
function resolveMovementPattern(exercise: WorkoutExercise): MovementPattern | undefined {
  return findExerciseInLibrary(exercise.name, exercise.label)?.movementPattern ?? inferPatternFromName(exercise.name);
}
...
export function buildWorkoutWarmup(exercises: WorkoutExercise[]): WorkoutWarmupPlan {
  ...
  const tip =
    rampExercises.length > 0 ? "Leave 2-3 reps in reserve on your last warm-up set." : undefined;
