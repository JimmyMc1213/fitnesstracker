...
async function provisionTestUser(sb) {
  ...
  const email = `maestro.rn10.signout.${stamp}@newyouai.test`;
  const password = `MaestroRn10O${String(stamp).slice(-8)}`;

  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: "Maestro RN10 SignOut" } },
  });
...
const result = spawnSync(
  "maestro",
  ["test", path.join(".maestro", "rn-auth-sign-out.yaml")],
  {
    cwd: mobileRoot,
    env: maestroEnv({
