#!/usr/bin/env node
/**
 * RN-6-11 workout-session Maestro — provisions a disposable Supabase user, then runs session smoke.
 * Requires: Metro on :8082 with EXPO_PUBLIC_E2E_FITNESS_SEED=workout-session, iOS simulator + dev client.
 */
...
async function provisionTestUser(sb) {
  ...
  const email = `maestro.rn6.${stamp}@newyouai.test`;
  const password = `MaestroRn6${String(stamp).slice(-8)}`;

  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: "Maestro RN6" } },
  });
...
console.log(`Maestro workout-session — test user ${creds.email}`);

const result = spawnSync(
  "maestro",
  ["test", path.join(".maestro", "rn-workout-session.yaml")],
  {
    cwd: mobileRoot,
    env: {
      ...process.env,
...
console.log("\nWorkout session Maestro passed.");
