...
async function provisionTestUser(sb) {
  ...
  const email = `maestro.rn7.${stamp}@newyouai.test`;
  const password = `MaestroRn7${String(stamp).slice(-8)}`;

  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: "Maestro RN7" } },
  });
...
const result = spawnSync(
  "maestro",
  ["test", path.join(".maestro", "rn-nutrition-log.yaml")],
  {
    cwd: mobileRoot,
    env: {
      ...process.env,
