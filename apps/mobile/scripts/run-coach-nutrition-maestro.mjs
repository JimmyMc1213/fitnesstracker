#!/usr/bin/env node
/**
 * RN-5-07 coach-nutrition Maestro — provisions a disposable Supabase user, then runs coach flow.
...
async function provisionTestUser(sb) {
  ...
  const email = `maestro.rn5.${stamp}@newyouai.test`;
  const password = `MaestroRn5${String(stamp).slice(-8)}`;

  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: "Maestro RN5" } },
  });
...
if (metroProbe.status !== 0) {
  console.error(
    "Metro is not reachable on http://127.0.0.1:8082 — start it first:\n" +
      "  cd apps/mobile && EXPO_PUBLIC_E2E_FITNESS_SEED=coach-nutrition npx expo start --dev-client --port 8082",
  );
  process.exit(1);
}
...
console.log(`Maestro coach-nutrition — test user ${creds.email}`);

const result = spawnSync(
  "maestro",
  ["test", path.join(".maestro", "rn-coach-nutrition.yaml")],
  {
    cwd: mobileRoot,
    env: {
      ...process.env,
...
console.log("\nCoach nutrition Maestro passed.");
