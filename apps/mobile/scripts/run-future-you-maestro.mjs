...
async function provisionTestUser(sb) {
  ...
  const email = `maestro.rn9.futureyou.${stamp}@newyouai.test`;
  const password = `MaestroRn9F${String(stamp).slice(-8)}`;

  const { data, error } = await sb.auth.signUp({
    email,
    password,
    options: { data: { full_name: "Maestro RN9 Future You" } },
  });
...
const result = spawnSync(
  "maestro",
  ["test", path.join(".maestro", "rn-future-you-upload.yaml")],
  {
    cwd: mobileRoot,
    env: {
      ...process.env,
