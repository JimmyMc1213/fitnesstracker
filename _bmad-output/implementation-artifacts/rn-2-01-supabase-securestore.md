...
- [ ] Maestro `rn-auth-gate.yaml` (requires Supabase-configured dev client)
- [ ] Maestro `rn-auth-gate-sign-in.yaml` (requires test account env vars)
...
- [ ] [Review][Patch] Maestro auth flows not executed — blocked locally (Java runtime missing); run on simulator with Supabase env before marking done
