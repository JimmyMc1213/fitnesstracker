/** Keep in sync with supabase/functions/_shared/future-you/staleJob.ts */

import type { FutureYouJobStatus } from "@newyouai/types";

/**
 * Jobs in queued/generating longer than this are treated as stuck.
 * Must stay safely above the edge function's generation timeout (130s) and
 * Supabase's ~150s hard request limit so a slow-but-valid generation is never
 * marked failed by the client/status reconcile while the server is still working.
 */
