import type { CompletedWorkoutSession, ExerciseSessionSnapshot, WeightUnit } from "@newyouai/types";

import { exerciseNoteKey } from "../workout/exerciseNoteKey";
import { formatSetWeight, weightUnitLabel } from "../workout/unitDisplay";
import { sessionBestForExercise } from "../workout/workoutSummarySession";
...
function annotatePrHistory(snapshots: ExerciseSessionSnapshot[]): PersonalRecordHistoryEntry[] {
  ...
  for (const snap of sorted) {
    ...
    const isPr = hadPrior && isBetterSet(snap.bestWeight, snap.bestReps, maxWeight, maxReps);
    ...
    if (isBetterSet(snap.bestWeight, snap.bestReps, maxWeight, maxReps)) {
      maxWeight = snap.bestWeight;
      maxReps = snap.bestReps;
    }
  }
