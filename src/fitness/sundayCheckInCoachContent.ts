import { MOBILITY_HABIT_ID } from "./mobilityHabit";
import { proteinGoalHitForDateKey } from "./dailyStreak";
import { exerciseNoteKey } from "./exerciseNotes";
import { formatWeeklySummaryRange } from "./weeklySummary";
...
export function buildSundayCheckInDayCells(state: AppState, weekKeys: string[]): SundayCheckInDayCell[] {
  return weekKeys.map((dateKey) => ({
    ...
    proteinHit: proteinGoalHitForDateKey(
...
export function buildSundayCheckInWatchItems(input: {
  ...
  const lowProteinDays = input.weekKeys.filter((dayKey) => {
    const hit = proteinGoalHitForDateKey(
